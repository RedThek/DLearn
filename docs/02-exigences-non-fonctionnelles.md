...
| NFR-33 | Le build de release ne contient aucun compte de démonstration ni identifiant connu du dépôt public | Installation à froid du build de release : aucun utilisateur préexistant ; revue du seed limité au build de débogage |
| NFR-34 | Tout fichier d'échange (bundle) et tout pack est vérifié par somme de contrôle avant import, appliqué de façon atomique, et détecté en cas de rejeu | Tests unitaires avec le vecteur de test de `20-…` ; test d'un fichier tronqué et d'un rejeu ; import interrompu sans état partiel |
| NFR-35 | L'identité des utilisateurs repose sur un `uid` immuable, jamais dérivé de données personnelles ; aucun identifiant matériel de l'appareil n'est collecté | Revue de code (aucun usage de `ANDROID_ID` ou équivalent) ; vérification que `uid` et `instanceId` sont des UUID aléatoires |
